...
/// <summary>
/// Adversarial constructor/lifecycle tests for <see cref="MongoDBMemoryIndexManager"/>'s connection-string
...
/// database/collection) is still disposed even though no <see cref="MongoDBMemoryIndexManager"/> instance is ever
/// returned to the caller (docs/spec/features/index-management.md's caller-owned-vs-manager-owned disposal
/// semantics).
/// </summary>
public sealed class MongoDBMemoryIndexManagerLifecycleTests
{
    [Fact]
    ...
    public void ConnectionStringConstructorValidatesArgumentsBeforeCreatingAClient()
    {
        ...
        // ConnectClient before MongoClientFactory.FromConnectionString ever runs; the client factory must never
        // be invoked, since a validation-only failure should never create (and therefore never need to dispose)
        // a client at all.
        Assert.Throws<MongoDBConfigurationException>(() => new MongoDBMemoryIndexManager(
            "mongodb://localhost:27017",
            databaseName: "   ",
            "chunks",
            new MongoDBVectorSearchIndexDefinition("facade_vector", "embedding", 3),
    ...
    [Fact]
    public void ConnectionStringConstructorValidatesTheDefinitionBeforeCreatingAClient()
    {
        bool clientFactoryInvoked = false;

        Assert.Throws<ArgumentNullException>(() => new MongoDBMemoryIndexManager(
            "mongodb://localhost:27017",
            "database",
            "chunks",
            definition: null!,
    ...
    public void ConnectionStringConstructorDisposesOwnedClientWhenLaterValidationFails()
    {
        ...
        Assert.Throws<InvalidOperationException>(() => new MongoDBMemoryIndexManager(
            "mongodb://localhost:27017",
            "database",
            "chunks",
            new MongoDBVectorSearchIndexDefinition("facade_vector", "embedding", 3),
        ...
        // The client was created by the factory before GetDatabase failed; since no MongoDBMemoryIndexManager
