...
.SYNOPSIS
    ...
    workflow-logic ref gate accepts only a trusted release-tag push or a workflow_dispatch run targeting
    'refs/heads/main' or a valid release tag, and rejects every other event/ref combination, including an
    arbitrary workflow_dispatch-selected feature branch and refs crafted with shell metacharacters.
...
Assert-Eligible "workflow_dispatch" "refs/tags/dotnet-v1.2.3" "workflow_dispatch targeting a valid release tag"
